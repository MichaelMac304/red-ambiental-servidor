...
async function cargarDatos() {
    ...
    try {
        ...
        for (const nodo in datos) {
            ...
            const esGateway = nodo === 'GATEWAY';

            const card = document.createElement('div');

            card.className = esGateway
                ? 'card gateway'
                : 'card';
...
1. Reemplazá completamente tu archivo `app.py` por este.

2. Volvé a hacer deploy en Render.

3. Abrí:

```text
https://red-ambiental-servidor.onrender.com
```
